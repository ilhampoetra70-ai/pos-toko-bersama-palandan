...
const CustomTooltip = ({ active, payload }: any) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-950 border dark:border-gray-800 rounded-2xl shadow-xl p-4 text-xs animate-in zoom-in-95">
            <p className="font-black text-gray-900 dark:text-gray-100 mb-2 uppercase tracking-widest">{d.name}</p>
            <div className="space-y-1">
                <p className="font-bold text-primary-600 dark:text-primary-400">Total: Rp {d.total?.toLocaleString('id-ID')}</p>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">{d.count} transaksi</p>
            </div>
        </div>
    );
};
    ...
    return (
        <Card className="border-none shadow-sm bg-white dark:bg-gray-950 rounded-[2rem] overflow-hidden">
            <CardHeader className="pb-2">
                <CardTitle className="text-xs font-black uppercase tracking-widest text-gray-400">Metode Pembayaran</CardTitle>
            </CardHeader>
            <CardContent>
