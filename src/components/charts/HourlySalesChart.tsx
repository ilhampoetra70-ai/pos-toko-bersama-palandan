...
const CustomTooltip = ({ active, payload }: any) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-950 border dark:border-gray-800 rounded-2xl shadow-xl p-4 text-xs animate-in zoom-in-95">
            <p className="font-black text-gray-900 dark:text-gray-100 mb-2 uppercase tracking-widest">{String(d.hour).padStart(2, '0')}:00</p>
            <div className="space-y-1">
                <p className="font-bold text-primary-600 dark:text-primary-400">Pendapatan: Rp {d.total?.toLocaleString('id-ID')}</p>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">{d.count} transaksi terdeteksi</p>
            </div>
        </div>
    );
};
    ...
    return (
        <Card className="border-none shadow-sm bg-white dark:bg-gray-950 rounded-[2rem] overflow-hidden">
            <CardHeader className="pb-2">
                <CardTitle className="text-xs font-black uppercase tracking-widest text-gray-400">Pola Penjualan per Jam</CardTitle>
            </CardHeader>
            <CardContent>
                    ...
                    <BarChart data={data} margin={{ top: 20, right: 10, left: 10, bottom: 0 }}>
                        ...
                        <Bar dataKey="total" radius={[6, 6, 6, 6]} barSize={24}>
                            {data.map((entry, i) => (
                                <Cell
                                    key={i}
                                    fill={entry.total >= maxTotal * 0.8 ? '#3b82f6' : '#94a3b8'}
                                    fillOpacity={entry.total >= maxTotal * 0.8 ? 1 : 0.2}
                                />
                            ))}
                        </Bar>
                    </BarChart>
