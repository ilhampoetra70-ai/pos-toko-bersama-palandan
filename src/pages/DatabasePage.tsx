    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto">
            <Dialog open={!!confirmModal} onOpenChange={(open) => !open && setConfirmModal(null)}>
                <DialogContent className="max-w-md">
                    ...
                    {confirmModal?.variant === 'hard-reset' && (
                        <div className="space-y-4 py-4">
                            ...
                            <div className="space-y-2">
                                <p className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Ketik RESET untuk konfirmasi</p>
                                <Input
                    ...
                    <DialogFooter className="gap-2 sm:gap-0 mt-4">
                        ...
                        >
                            {confirmModal?.variant === 'hard-reset' ? <Trash2 className="w-4 h-4" /> : <Check className="w-4 h-4" />}
                            Ya, Lanjutkan
                        </Button>
                    </DialogFooter>
                </DialogContent>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h2 className="text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight">Manajemen Database</h2>
                    <p className="text-sm text-gray-500 font-medium">Pemeliharaan, backup, dan ekspor data sistem</p>
                </div>
                <div className="flex items-center gap-3">
                    {message && (
                        <Badge className={cn(
                            "h-9 px-4 font-bold shadow-none animate-in fade-in slide-in-from-top-2 duration-300",
                            message.type === 'error' ? "bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-400" : "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400"
                        )}>
                            {message.text}
                        </Badge>
                    )}
                    <Button variant="outline" onClick={() => refetchStats()} disabled={isLoadingStats} className="h-11 px-6 shadow-sm font-bold gap-2">
                        <RefreshCw className={cn("w-4 h-4", isLoadingStats && "animate-spin")} /> Refresh Stats
                    </Button>
                </div>
            </div>

            <Tabs defaultValue="stats" onValueChange={setActiveTab} className="w-full">
                <TabsList className="bg-gray-100 dark:bg-gray-800 p-1 h-14 rounded-2xl w-full sm:w-auto overflow-x-auto sm:overflow-visible flex-nowrap">
                ...
                <div className="mt-6">
                    <TabsContent value="stats" className="space-y-6">
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <Card className="lg:col-span-2 border-none shadow-sm overflow-hidden bg-white dark:bg-gray-900">
                                <CardHeader className="bg-gray-50/50 dark:bg-gray-800/50 border-b dark:border-gray-800 h-16 flex flex-row items-center justify-between py-0">
                                    <CardTitle className="text-sm font-black uppercase tracking-widest text-gray-500">Jumlah Record Data</CardTitle>
                                ...
                                <CardContent className="p-0">
                                    <Table>
                                        <TableBody className="divide-y dark:divide-gray-800">
                                            {stats ? [
                                                { label: 'Total Pengguna', value: stats.counts.users, icon: <Users className="w-4 h-4 text-blue-500" /> },
                                                { label: 'Kategori Produk', value: stats.counts.categories, icon: <Archive className="w-4 h-4 text-purple-500" /> },
                                                { label: 'Daftar Produk', value: stats.counts.products, icon: <Package className="w-4 h-4 text-emerald-500" /> },
                                                { label: 'Transaksi Selesai', value: stats.counts.transactions, icon: <ShoppingCart className="w-4 h-4 text-orange-500" /> },
                                                { label: 'Detil Item Terjual', value: stats.counts.transaction_items, icon: <FileText className="w-4 h-4 text-gray-400" /> },
                            ...
                            <div className="space-y-6">
                                <Card className="border-none shadow-sm bg-white dark:bg-gray-900">
                                    <CardHeader className="h-16 flex flex-row items-center justify-between py-0 border-b dark:border-gray-800">
                                        <CardTitle className="text-sm font-black uppercase tracking-widest text-gray-500">Info Filesystem</CardTitle>
                                        <HardDrive className="w-5 h-5 text-gray-400" />
                                    </CardHeader>
                                    <CardContent className="p-6 space-y-4">
                                        <div className="flex justify-between items-end border-b pb-3 dark:border-gray-800">
                                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Ukuran File DB</span>
                                            <span className="text-xl font-black font-mono">{stats ? formatFileSize(stats.fileSize) : '-'}</span>
                                        </div>
                                        <div className="flex justify-between items-end border-b pb-3 dark:border-gray-800">
                                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Backup Terakhir</span>
                                            <span className="text-xs font-bold text-gray-600">{stats?.lastBackupDate ? formatDate(stats.lastBackupDate) : '-'}</span>
                                        </div>
                                        <div className="flex justify-between items-end">
                                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Transaksi Tertua</span>
                                            <span className="text-xs font-bold text-gray-600">{stats?.oldestTransaction || '-'}</span>
                                        </div>
                                    </CardContent>
                                </Card>
                                ...
                                <Card className="border-none shadow-sm bg-primary-600 text-white">
                                    ...
                                    <CardContent className="pb-6">
                                        ...
                                        <Button
                                            ...
                                            disabled={processing}
                                            className="w-full bg-white text-primary-600 hover:bg-white/90 font-black h-11 shadow-lg"
                                        >
                    ...
                    <TabsContent value="backup" className="space-y-6">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <Card className="border-none shadow-sm bg-white dark:bg-gray-900 border-l-4 border-l-primary-500">
                                <CardHeader>
                            ...
                            <Card className="border-none shadow-sm bg-white dark:bg-gray-900 border-l-4 border-l-orange-500">
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-3 text-xl font-black uppercase tracking-tight">
                                        <RefreshCw className="w-6 h-6 text-orange-600" /> Pulihkan Data
                                    </CardTitle>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <Button onClick={handleRestoreBackup} disabled={processing} className="w-full h-14 bg-orange-600 hover:bg-orange-700 font-black text-lg gap-3 shadow-lg shadow-orange-600/20">
                                        <History className="w-5 h-5" /> Pilih File Backup...
                                    </Button>
                                </CardContent>
                            </Card>
                        </div>

                        <Card className="border-none shadow-sm overflow-hidden bg-white dark:bg-gray-900 mt-6">
                            <CardHeader className="bg-gray-50/50 dark:bg-gray-800/50 border-b dark:border-gray-800 h-16 flex flex-row items-center justify-between py-0">
                                <CardTitle className="text-sm font-black uppercase tracking-widest text-gray-500 flex items-center gap-2">
                                    <History className="w-4 h-4" /> Riwayat Pencadangan Lokal
                                </CardTitle>
                            </CardHeader>
                            <CardContent className="p-0">
                                <Table>
                                    <TableHeader>
                                        <TableRow className="h-12 border-b dark:border-gray-800">
                                            <TableHead className="font-black text-[10px] uppercase tracking-widest pl-10">NAMA FILE CADANGAN</TableHead>
                                            <TableHead className="font-black text-[10px] uppercase tracking-widest text-center">TANGGAL DIBUAT</TableHead>
                                            <TableHead className="font-black text-[10px] uppercase tracking-widest text-right">UKURAN</TableHead>
                                            <TableHead className="font-black text-[10px] uppercase tracking-widest text-right pr-10">AKSI</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody className="divide-y dark:divide-gray-800">
                    ...
                    <TabsContent value="maintenance" className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <Card className="md:col-span-2 border-none shadow-sm bg-white dark:bg-gray-900 overflow-hidden">
                                <CardHeader className="pb-2">
                                ...
                                <CardContent className="space-y-6">
                                    <div className="flex flex-col sm:flex-row items-end gap-6 p-6 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border dark:border-gray-800">
                                        <div className="space-y-2 flex-1">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Retensi Data (Bulan)</label>
                                            <Input
                                                type="number"
                                                min="1"
                                                value={archiveMonths}
                                    ...
                                    {archivableCount != null && (
                                        <div className="px-6 flex items-center gap-3">
                                            ...
                                            <span className="text-sm font-bold text-gray-500 italic">Ditemukan data sebelum {archivableCount.cutoffDate}</span>
                                        </div>
                                    )}
                                </CardContent>
                            </Card>

                            <Card className="lg:col-span-1 border-4 border-red-500/20 shadow-sm bg-white dark:bg-gray-900">
                                <CardHeader>
                                    <CardTitle className="text-xl font-black text-red-600 uppercase flex items-center gap-2">
                                        <AlertTriangle className="w-5 h-5" /> Hard Reset
                                    </CardTitle>
                                </CardHeader>
                                <CardContent className="pb-6">
                    ...
                    <TabsContent value="export" className="space-y-6">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <Card className="border-none shadow-sm bg-white dark:bg-gray-900">
                                ...
                                <CardContent className="space-y-6">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Dari Tanggal</label>
                                            <Input type="date" className="h-11 font-bold border-gray-100" />
                                        </div>
                                        <div className="space-y-2">
                                    ...
                                    <Button className="w-full h-14 bg-gray-900 hover:bg-black font-black text-lg gap-3">
                            ...
                            <Card className="border-none shadow-sm bg-white dark:bg-gray-900">
                                <CardHeader>
                                    <CardTitle className="text-xl font-black uppercase tracking-tight flex items-center gap-2">
                                        <FileText className="w-6 h-6 text-red-600" /> Ringkasan Database PDF
                                    </CardTitle>
...
function MaintenanceCard({ title, description, icon: Icon, label, bottomLabel, action, btnText, processing, disabled, variant }: any) {
    return (
        <Card className="border-none shadow-sm bg-white dark:bg-gray-900 flex flex-col justify-between">
            <CardHeader className="pb-3">
                <CardTitle className="text-lg font-black uppercase tracking-widest text-gray-500 flex items-center gap-2">
