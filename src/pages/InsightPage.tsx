...
function ModelSettingsPanel({ customModelPath, browseError, isChangingModel, aiMode, onBrowse, onClear }: {
    ...
    useEffect(() => {
        ...
        window.api.getAiApiSettings().then((s) => {
            if (s.success) { setApiSettings({ mode: s.mode as 'local' | 'api', provider: s.provider, apiKey: s.apiKey, model: s.model, baseUrl: s.baseUrl }); setActiveTab(s.mode === 'api' ? 'api' : 'local'); }
        });
    }, [open]);
                ...
                <div className="border-t border-border dark:border-border">
                    ...
                    {activeTab === 'api' && (
                        <div className="px-5 py-4 space-y-4">
                            ...
                            <div><label className="text-xs font-semibold text-muted-foreground dark:text-muted-foreground block mb-1.5">Nama Model</label>
                                {currentProvider.models.length > 0 ? <select value={apiSettings.model || currentProvider.defaultModel} onChange={e => setApiSettings(s => ({ ...s, model: e.target.value }))} className="w-full px-3 py-2 text-sm rounded-lg border border-border dark:border-border bg-card dark:bg-card text-foreground dark:text-foreground focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-400">{currentProvider.models.map(m => <option key={m} value={m}>{m}</option>)}</select>
