        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 animate-in fade-in duration-300">
            <div className="bg-card dark:bg-background rounded-3xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                <div className="p-6 border-b dark:border-border flex items-center justify-between bg-background dark:bg-background">
                    <div>
                        <h3 className="font-black text-xl text-foreground dark:text-foreground uppercase tracking-tight">Bayar Piutang</h3>
                        <p className="text-xs text-muted-foreground font-medium">Tambah riwayat cicilan atau pelunasan</p>
                    </div>
                    <button
                        onClick={onClose}
                        className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-red-50 dark:hover:bg-red-950/30 text-muted-foreground hover:text-red-500 transition-all"
                    >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="bg-primary-50 dark:bg-primary-950/30 rounded-2xl p-4 border border-primary-100 dark:border-primary-900/50 flex items-center justify-between">
                        <div>
                            ...
                            {transaction.customer_name && (
                                <div className="text-sm text-muted-foreground dark:text-muted-foreground font-bold mt-0.5">{transaction.customer_name}</div>
                            )}
                        </div>
                    ...
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest px-1">Metode Pembayaran</label>
                            ...
                            {['cash', 'debit', 'qris'].map(method => (
                                <button
                                    ...
                                    className={`flex-1 py-3 rounded-2xl text-xs font-black uppercase tracking-tight border-2 transition-all ${paymentMethod === method
                                        ...
                                        : 'border-border dark:border-border text-muted-foreground dark:text-muted-foreground hover:border-border dark:hover:border-gray-700 bg-background dark:bg-background hover:bg-muted dark:hover:bg-card'
                                        }`}
                                >
                    ...
                    {/* Amount */}
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest px-1">Jumlah Pembayaran</label>
                        <div className="relative">
                            <input
                                type="number"
                                className="w-full bg-background dark:bg-background border-none rounded-2xl h-16 text-2xl font-black text-center text-foreground dark:text-foreground px-12 shadow-inner focus:ring-2 focus:ring-primary-500/20 transition-all"
                                value={amount}
                                ...
                                autoFocus
                            />
                            <div className="absolute left-6 top-5 text-muted-foreground font-black text-lg">Rp</div>
                        </div>
                    </div>
                        ...
                        {quickAmounts.map((amt, i) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${amt === remainingBalance
                                    ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-400 border border-green-200 dark:border-green-800 hover:bg-green-200'
                    ...
                    {/* Notes */}
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest px-1">Catatan</label>
                        <textarea
                            className="w-full bg-background dark:bg-background border-none rounded-2xl p-4 text-sm font-medium resize-none shadow-inner"
                            value={notes}
                    ...
                    {/* Payment Summary */}
                    {paymentAmount > 0 && (
                        <div className={`rounded-2xl p-4 transition-colors ${isFullPayment ? 'bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900/30' : 'bg-primary dark:bg-primary/20 border border-primary dark:border-primary/30'}`}>
                            <div className="space-y-2">
                                <div className="flex justify-between items-center text-xs">
                                    <span className={isFullPayment ? 'text-green-600 dark:text-green-400' : 'text-primary dark:text-primary'}>Pembayaran Masuk</span>
                                    <span className="font-black text-foreground dark:text-foreground">{formatCurrency(paymentAmount)}</span>
                                </div>
                                <div className="flex justify-between items-center pt-2 border-t border-border dark:border-border">
                                    <span className={isFullPayment ? 'text-green-700 dark:text-green-300' : 'text-primary dark:text-primary'}>Sisa Piutang Akhir</span>
                                    <span className="font-black text-lg text-foreground dark:text-foreground">{formatCurrency(remainingBalance - paymentAmount)}</span>
                                </div>
                            </div>
                            {isFullPayment && (
                ...
                <div className="p-6 border-t dark:border-border bg-background dark:bg-background flex gap-4">
                    <button onClick={onClose} className="flex-1 h-12 rounded-2xl font-bold text-muted-foreground dark:text-muted-foreground hover:bg-muted dark:hover:bg-card transition-colors">
                        Tutup
                    </button>
                    <button
