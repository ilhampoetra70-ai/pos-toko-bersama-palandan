...
contextBridge.exposeInMainWorld('api', {
  ...
  markPasswordChanged: (id) => ipcRenderer.invoke('users:markPasswordChanged', id),
  ...
  validateProductsActiveBulk: (productIds) => ipcRenderer.invoke('products:validateActiveBulk', productIds),
  ...
  dbVacuum: (userId) => ipcRenderer.invoke('db:vacuum', userId),
  dbClearVoided: (userId) => ipcRenderer.invoke('db:clearVoided', userId),
  dbGetArchivableCount: (months) => ipcRenderer.invoke('db:getArchivableCount', months),
  dbArchiveTransactions: (months, userId) => ipcRenderer.invoke('db:archiveTransactions', months, userId),
  dbResetSettings: () => ipcRenderer.invoke('db:resetSettings'),
  dbHardReset: (userId) => ipcRenderer.invoke('db:hardReset', userId),
  dbGetBackupHistory: () => ipcRenderer.invoke('db:getBackupHistory'),
  dbCreateBackup: () => ipcRenderer.invoke('db:createBackup'),
  dbDeleteBackup: (filePath, userId) => ipcRenderer.invoke('db:deleteBackup', filePath, userId),
  dbManualBackup: () => ipcRenderer.invoke('db:manualBackup'),
  dbRestoreBackup: (userId) => ipcRenderer.invoke('db:restoreBackup', userId),
  dbRestoreFromHistory: (filePath, userId) => ipcRenderer.invoke('db:restoreFromHistory', filePath, userId),
  ...
  startAiDownload: () => ipcRenderer.invoke('ai:startDownload'),
  ...
  onAiAutoGenerating: (callback) => {
    ipcRenderer.on('ai:autoGenerating', (_, data) => callback(data));
    return () => ipcRenderer.removeAllListeners('ai:autoGenerating');
  },
  onAiAutoGenerateDone: (callback) => {
    ipcRenderer.on('ai:autoGenerateDone', (_, data) => callback(data));
    return () => ipcRenderer.removeAllListeners('ai:autoGenerateDone');
  },
});
