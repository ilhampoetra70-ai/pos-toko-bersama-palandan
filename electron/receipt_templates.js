const templates = {
  ...
  '58mm-9': {
    ...
    render: (data) => `
      <style>
        body { font-family: 'Courier New', monospace; font-size: ${data.fontSize}; width: ${data.widthPx}px; padding: 4px 6px; line-height: 1.25; color: #000; }
        .center { text-align: center; }
        .right  { text-align: right; }
        .bold   { font-weight: bold; }
        .sm     { font-size: 0.88em; }
  ...
  '58mm-10': {
    ...
    render: (data) => `
      <style>
        body { font-family: 'Courier New', monospace; font-size: ${data.fontSize}; width: ${data.widthPx}px; padding: 4px 6px; line-height: 1.3; color: #000; }
        .center { text-align: center; }
        .right  { text-align: right; }
        .bold   { font-weight: bold; }
        .sm     { font-size: 0.88em; }
          ...
          ${data.items.map(item => `
            <tr>
              <td class="c-name" style="padding:2px 0;">
                ${item.name}
                <br/><span class="sm">@ ${data.formatCurrency(item.price)}</span>
              </td>
              <td class="c-qty" style="padding:2px 0; text-align:center;">${item.qty}</td>
              <td class="c-total" style="padding:2px 0;">${data.formatCurrency(item.subtotal)}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  '58mm-12': {
    ...
    render: (data) => `
      <style>
        body { font-family: 'Courier New', monospace; font-size: ${data.fontSize}; width: ${data.widthPx}px; padding: 3px 5px; line-height: 1.2; color: #000; }
        .center { text-align: center; }
        .right  { text-align: right; }
        .bold   { font-weight: bold; }
        .sm     { font-size: 0.85em; }
        ...
        <tbody>
          ${data.items.map(item => `
            <tr style="padding: 1px 0;">
              ...
              <td class="c-qty" style="padding:2px 0; text-align:center;">${item.qty}</td>
              <td class="c-sub" style="padding:2px 0;">${data.formatCurrency(item.subtotal)}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  // Printable width: ~217mm (8.5" after tractor holes) = 820px
  // Font sizes scaled up: sm=11px, md=13px, lg=15px
  // ==========================================

  // CF-1: Standar CF — centered store header, two-column invoice info,
  // 5-col items table (inverted black header, alternating rows), right-aligned summary
  'cf-1': {
    name: 'Standar CF',
    width: 'cf',
    render: (data) => {
      ...
      return `
        <style>
          body { font-family: Arial, Helvetica, sans-serif; font-size: ${fs}; width: ${data.widthPx}px; padding: 15px 20px; line-height: 1.5; color: #000; }
          ...
          .store-header { text-align: center; padding-bottom: 10px; }
          .store-name { font-size: 1.5em; font-weight: bold; margin: 0 0 4px; }
          .store-sub  { font-size: 0.9em; }
          .two-col { display: flex; justify-content: space-between; gap: 20px; margin: 10px 0; }
          .two-col > div { flex: 1; }
          .kv { width: 100%; border-collapse: collapse; }
          .kv td.k { font-weight: bold; width: 38%; white-space: nowrap; padding: 1px 0; }
          .kv td.sep { width: 8px; padding: 1px 0; }
          .kv td.v { padding: 1px 0; }
          table.items { width: 100%; border-collapse: collapse; margin: 10px 0; }
          table.items thead th { background: #000; color: #fff; padding: 7px 8px; text-align: left; }
          table.items thead th.right  { text-align: right; }
          table.items thead th.center { text-align: center; }
          table.items tbody tr:nth-child(even) { background: #f5f5f5; }
          table.items tbody td { padding: 5px 8px; border-bottom: 1px solid #e0e0e0; vertical-align: top; }
          table.items tbody td.right  { text-align: right; }
          table.items tbody td.center { text-align: center; }
          table.items tfoot td { padding: 6px 8px; border-top: 2px solid #000; font-weight: bold; }
          ...
          table.sum .total td { font-weight: bold; font-size: 1.1em; border-top: 2px solid #000; padding-top: 6px; }
          table.sum .pay td { font-size: 0.92em; }
          .footer { border-top: 1px dashed #000; padding-top: 8px; margin-top: 15px; text-align: center; font-size: 0.9em; }
        </style>
  ...
  // CF-2: Invoice Bisnis — store info left, bordered invoice box right,
  // serif font, table with double-border header/footer, summary in bordered box
  'cf-2': {
    name: 'Invoice Bisnis',
    width: 'cf',
    render: (data) => {
      ...
      return `
        <style>
          body { font-family: 'Times New Roman', Georgia, serif; font-size: ${fs}; width: ${data.widthPx}px; padding: 15px 20px; line-height: 1.5; color: #000; }
          .right { text-align: right; } .center { text-align: center; } .bold { font-weight: bold; }
          hr.thick { border: none; border-top: 2px solid #000; margin: 10px 0; }
